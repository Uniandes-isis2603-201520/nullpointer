...
import co.edu.uniandes.nullpointer.rest.tripulator.resources.FotoResource;
...
public class FotoResourceIT {
    ...
    private final String fotoPath = "/viajeros/1/itinerarios/1/fotos";

    private final static List<FotoDTO> oraculo = new ArrayList<>();
    ...
    public static WebArchive createDeployment() {
        return ShrinkWrap.create(WebArchive.class)
                ...
                .addPackage(FotoDTO.class.getPackage())
                .addPackage(FotoConverter.class.getPackage())
    ...
    public void createFotoTest() throws IOException {
        FotoDTO foto = oraculo.get(0);

        Response response = target.path(fotoPath).request()
                .post(Entity.entity(foto, MediaType.APPLICATION_JSON));

        FotoDTO fotoTest = (FotoDTO) response.readEntity(FotoDTO.class);

        Assert.assertEquals(CREATED, response.getStatus());
        Assert.assertEquals(foto.getSrc(), fotoTest.getSrc());
    }

    
    @Test
    @InSequence(3)
    public void listFotoTest() throws IOException {

        Response response = target.path(fotoPath)
                .request().get();
        String listFoto = response.readEntity(String.class);
        List<FotoDTO> listFotoTest = new ObjectMapper().readValue(listFoto, List.class);
        Assert.assertEquals(OK, response.getStatus());
        Assert.assertEquals(1, listFotoTest.size());
    }
    ...
    public void deleteFotoTest() {
        ...
        FotoDTO foto = oraculo.get(0);
        Response response = target.path(fotoPath).path(foto.getId().toString())
                .request().delete();
