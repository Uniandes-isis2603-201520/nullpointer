...
public class ItinerarioResourceIT {
    ...
    private final String itinerarioPath = "/viajeros/1/itinerarios";
    ...
    @Test
    @InSequence(1)
    public void createItinerarioTest() throws IOException {
        ...
        Assert.assertEquals(itinerario.getNombre(), itinerarioTest.getNombre());
    }
    ...
    public void getItinerarioById() {
        ...
        Assert.assertEquals(itinerarioTest.getId(), oraculo.get(0).getId());
        Assert.assertEquals(itinerarioTest.getNombre(), oraculo.get(0).getNombre());
    }

    @Test
    @InSequence(3)
    public void listItinerarioTest() throws IOException {

        Response response = target.path(itinerarioPath)
                .request().get();
        String listItinerario = response.readEntity(String.class);
        List<ItinerarioDTO> listItinerarioTest = new ObjectMapper().readValue(listItinerario, List.class);
    ...
    @Test
    @InSequence(4)
    public void updateItinerarioTest() throws IOException {

        ItinerarioDTO itinerario = oraculo.get(0);

        ItinerarioDTO editorialChanged = factory.manufacturePojo(ItinerarioDTO.class);
        itinerario.setNombre(editorialChanged.getNombre());
        Response response = target.path(itinerarioPath).path(itinerario.getId().toString())
